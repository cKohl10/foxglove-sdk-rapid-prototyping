...
def preload_camera_data(cam, calib_path, img_dir):
    ...
    M_in = intrinsics.item().get(cam.idx) # Intrinsic matrix as [K | 0]
    M_ex = extrinsics.item().get(cam.idx)[0] # Extrinsic matrix as [R_3x3, t_3x1; 0_1x3, 1]
    ...
    cam.calibrate_mono_camera(M_in, M_ex, frame_width, frame_height, start_timestamp_nsec)
